...
import { getOrders, saveOrders, clearOrders } from './storage.js';
...
export async function runSync(options = {}) {
    ...
    if (ordersMissingDetails.length > 0 && !interrupted) {
    ...
    if (newOrders.length > 0 || detailsFetched > 0) {
        ...
        if (interrupted) {
            console.log(`Sync interrupted. Saved ${newOrders.length} new orders and ${detailsFetched} details fetched so far.`);
        } else {
