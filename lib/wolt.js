...
export async function validateToken(token) {
    try {
        ...
        return { valid: true, orderCount: response.data.orders ? response.data.orders.length : 0 };
    } catch (error) {
        if (error.response) {
            return { valid: false, status: error.response.status, message: error.response.statusText };
        }
